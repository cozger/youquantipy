...
def face_detection_process(detection_queue, detection_result_queue, 
                          ...
                          enable_gpu_ipc: bool = False, gpu_device_id: int = 0):
    """Face detection - GPU ONLY VERSION with frame ID synchronization and GPU IPC support"""
    print("[FACE DETECTOR] Starting GPU-ONLY face detection")

    frames_received = 0
    gpu_frames_received = 0
    ...
    # Initialize GPU memory client for IPC if enabled
    gpu_memory_client = None
    if enable_gpu_ipc:
            try:
                gpu_memory_client = create_gpu_memory_client(gpu_device_id)
                if gpu_memory_client:
                    print("[FACE DETECTOR] GPU IPC client initialized successfully")
                    # Test the client
                    test_handle = GPUMemoryHandle(b'test', (1, 1), np.float32, gpu_device_id)
                    test_array = gpu_memory_client.get_array(test_handle)
                    if test_array is None:
                        print("[FACE DETECTOR] GPU IPC client test failed")
                else:
                    print("[FACE DETECTOR] Failed to create GPU IPC client")
                    enable_gpu_ipc = False
            except Exception as e:
                print(f"[FACE DETECTOR] GPU IPC initialization error: {e}")
                import traceback
                traceback.print_exc()
                enable_gpu_ipc = False
                gpu_memory_client = None
    ...
    while True:
        ...
        while True:
            try:
                ...
                frames_received += 1
                # Debug frame type
                if 'is_gpu_frame' in new_frame and new_frame['is_gpu_frame']:
                    gpu_frames_received += 1
                    if gpu_frames_received % 10 == 0:
                        print(f"[FACE DETECTOR] Received GPU frame {gpu_frames_received}")
        ...
        if frame_data is not None:
            ...
            if frame_count % 30 == 0:
                print(f"[FACE DETECTOR] Stats: frames_received={frames_received}, "
                      f"gpu_frames={gpu_frames_received}, processed={frame_count}")
            
            
            # Extract frame ID first to track actual frame numbers
            frame_id = frame_data['frame_id']
            is_gpu_frame = frame_data.get('is_gpu_frame', False)
            
            if is_gpu_frame and gpu_memory_client and 'gpu_handles' in frame_data:
                print(f"[FACE DETECTOR] Processing GPU frame {frame_id}")
            ...
            if is_gpu_frame and gpu_memory_client and 'gpu_handles' in frame_data:
                # Handle GPU memory frames
                gpu_handles = frame_data['gpu_handles']
                if 'rgb_detection' in gpu_handles:
                    # Get GPU array from handle
                    detection_handle = gpu_handles['rgb_detection']
                    gpu_array = gpu_memory_client.get_array(detection_handle)
                    if gpu_array is not None:
                        # Convert to CPU for current detector (future: keep on GPU)
                        import cupy as cp
                        rgb_detection = cp.asnumpy(gpu_array)
                    else:
                        print(f"[FACE DETECTOR] Failed to get GPU array from handle")
                        continue
                else:
                    # Full resolution GPU handle
                    rgb_handle = gpu_handles['rgb']
                    gpu_array = gpu_memory_client.get_array(rgb_handle)
                    if gpu_array is not None:
                        import cupy as cp
                        rgb_detection = cp.asnumpy(gpu_array)
                    else:
                ...
                detection_scale_x = frame_data.get('detection_scale_x', 1.0)
                detection_scale_y = frame_data.get('detection_scale_y', 1.0)
                detection_offset_x = frame_data.get('detection_offset_x', 0)
                detection_offset_y = frame_data.get('detection_offset_y', 0)
            else:
                # Handle CPU frames
...
def face_worker_process(frame_queue, result_queue, model_path: str,
    ...
    while True:
        ...
        # Check if we have GPU handles for this frame
        gpu_handle = None
        if is_gpu_frame and 'gpu_handles' in frame_data and 'rgb' in frame_data['gpu_handles']:
            gpu_handle = frame_data['gpu_handles']['rgb']
        
        for track_dict in tracked_objects:
            # Get ROI
            try:
                # Pass GPU handle if available, otherwise pass CPU frame
                frame_or_handle = gpu_handle if gpu_handle else rgb
...
def parallel_participant_worker(cam_idx, face_model_path, pose_model_path,
    ...
    frame_distributor = FrameDistributor(
        ...
        fps=fps,
        enable_gpu_sharing=enable_gpu_ipc,
        gpu_device_id=gpu_device_id
    )
    ...
    face_proc = Process(
        ...
        args=(face_frame_queue, face_result_queue, face_model_path,
              ...
              3, resolution, max_participants, enable_gpu_ipc, gpu_device_id)
    )
    face_proc.start()
